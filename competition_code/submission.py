...
from virgile import pure_pursuit_test_claude
...
class RoarCompetitionSolution:
    ...
    async def initialize(self) -> None:
        ...
        self.pure_pursuit = pure_pursuit_test_claude(np.array([waypoint.location for waypoint in self.maneuverable_waypoints]))
    ...
    async def step(self) -> None:
        ...
        look_ahead = self.pure_pursuit.adaptive_preview_distance(0.8 * vehicle_velocity_norm,
                                                                 vehicle_velocity_norm * 0.05, vehicle_velocity_norm)

        goalpt, lastFoundIndex, pure_steer_control = self.pure_pursuit.step(vehicle_location[:2], vehicle_rotation[2],look_ahead, self.current_waypoint_idx, vehicle_velocity_norm)
        ...
        if self.current_section in [27, 28]:
            print("mutipler1")
            steerMultiplier = 1.4
        elif self.current_section in range(37, 43):
            print("mutipler2")
            steerMultiplier = 1.4
        ...
        if self.current_section in [52]:
            throttle = 1

        control = {
            "throttle": np.clip(throttle, 0, 1),
            "steer": np.clip(pure_steer_control * steerMultiplier , -1, 1),
...
Current waypoint index: {self.current_waypoint_idx} in sector {self.current_section}\n"
    ...
    def average_point(self, current_speed):
        ...
        if self.current_section in [9, 10]:
            ...
            num_points = lookahead_value * 2
        elif self.current_section in range(37, 43, 1):
            num_points = int(0.9*current_speed)
